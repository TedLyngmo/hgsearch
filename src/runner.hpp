...
int run([[maybe_unused]] const std::string_view& program, [[maybe_unused]] const std::string_view& tsvname, std::istream& tsvfile,
    ...
    std::for_each(std::execution::par_unseq, basepairs.begin(), basepairs.end(), [&chrs, &result](auto& bpline) {
        // count hits
        ...
        std::for_each(std::execution::par_unseq, chrs.begin(), chrs.end(),
            [&bp, &count](const auto& chromosome) {
                const char* chr_b = &chromosome[0];
                const char* chr_e = &chromosome[0] + chromosome.size();
                for(;chr_b != chr_e && (chr_b = (const char*) memmem(chr_b, chr_e - chr_b, &bp[0], bp.size())) != nullptr; chr_b += bp.size()) {
                    ++count;
                }
        });
#else
